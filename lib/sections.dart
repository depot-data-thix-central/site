...
class _PublicPageState extends State<PublicPage> {
  ...
  void initState() {
    super.initState();
    _consent.load().then((c) {
      if (mounted) {
        setState(() => _consentChoice = c);
      }
    });
  ...
  @override
  void dispose() {
    _ctrl.removeListener(_onContent);
    _ctrl.dispose();
    super.dispose();
  }
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                _ctrl.error ?? 'Site en cours de configuration.',
...
class Navbar extends StatelessWidget {
            ...
            children: [
              ...
              if (desktop) ...[
                ...
                for (var i = 0; i < links.length; i++)
                  links[i].target == 'solutions' ? _solutionsMenu() : _link(links[i], i == 0),
                const Spacer(),
                _lang(),
                const SizedBox(width: 12),
                GoldButton(label: content.nav.ctaLabel, onTap: () => onNavigate(SectionId.contact)),
              ] else ...[
                const Spacer(),
                _lang(),
                IconButton(
  ...
  Widget _logo() => GestureDetector(
        onTap: () => onNavigate(SectionId.home),
        child: Row(
          children: [
            Container(
              width: 38,
              ...
              child: const Center(
                child: Text(
                  'S',
                  style: TextStyle(
                      color: AppColors.primaryDark, fontWeight: FontWeight.w900, fontSize: 20),
                ),
              ),
            ),
  ...
  Widget _link(NavLink l, bool active) => TextButton(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
                height: 2, width: 24, color: active ? AppColors.gold : Colors.transparent),
          ],
        ),
      );
  ...
  Widget _solutionsMenu() => PopupMenuButton<String>(
        ...
        itemBuilder: (_) => [
          for (final s in content.solutions.main)
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              Text(
                content.nav.links
                    .firstWhere((l) => l.target == 'solutions',
                        orElse: () => NavLink(label: 'Nos solutions', target: 'solutions'))
                    .label,
  ...
  Widget _lang() => PopupMenuButton<String>(
        onSelected: (_) {},
        itemBuilder: (_) => const [
              ...
              for (final l in links)
                ListTile(
                  ...
                  onTap: () {
                    ...
                    onNavigate(_sectionOf(l.target));
                  },
                ),
            ],
          ),
...
class HeroSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: CustomPaint(
        ...
        child: R.centered(
          Padding(
            padding: EdgeInsets.symmetric(vertical: desktop ? 56 : 40),
            child: desktop
                ? Row(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: [
                      Expanded(flex: 5, child: _text(context)),
        ...
        children: [
          RichText(
            text: TextSpan(
              ...
              children: [
                for (var i = 0; i < h.eyebrow.length; i++) ...[
          ...
          RichText(
            text: TextSpan(
              ...
              children: [
                ...
                TextSpan(text: h.highlight, style: const TextStyle(color: AppColors.gold)),
              ],
            ),
          ),
          const SizedBox(height: 18),
  ...
  Widget _visual(BuildContext context) => LayoutBuilder(builder: (context, c) {
        ...
        return SizedBox(
          height: showPhone ? 430 : 300,
          child: Stack(
            clipBehavior: Clip.none,
            children: [
              if (showPhone) const Positioned(right: 0, top: 0, child: PhoneMock()),
              Positioned(
                  left: 0, top: showPhone ? 110 : 0, child: ThixCardMock(card: h.card, width: cardW)),
              if (showPhone)
                Positioned(
                  right: 6,
                  top: -14,
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(
                            fontSize: 10,
                            letterSpacing: 1.2,
                            color: AppColors.goldLight.withValues(alpha: .85),
                            fontWeight: FontWeight.w700),
                      ),
                    ],
                  ),
                ),
              Positioned(left: -12, top: showPhone ? 170 : 60, child: _nfc(h.card.nfc)),
            ],
          ),
        );
      });

  Widget _nfc(String label) => Column(
        children: [
          Container(
            width: 44,
            height: 44,
          ...
          Text(label,
              style: const TextStyle(
                  color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 1)),
        ],
      );
}
...
class PhoneMock extends StatelessWidget {
  ...
  static const _rows = [
    (Icons.folder_outlined, 'Mes documents'),
    (Icons.person_outline, 'Mon profil'),
    (Icons.verified_outlined, 'Mes certifications'),
    (Icons.work_outline, 'Mes opportunités')
  ];
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [
            BoxShadow(color: Color(0x88000000), blurRadius: 34, offset: Offset(0, 18))
          ],
        ),
        child: Column(
          children: [
            Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    for (final i in [Icons.signal_cellular_alt, Icons.wifi, Icons.battery_full])
            ...
            const Row(
              children: [
                ...
                Spacer(),
                Icon(Icons.menu, color: Colors.white54, size: 14),
              ],
            ),
            const SizedBox(height: 10),
            Container(
                      ...
                      Row(
                        children: [
                          ...
                          Text('Identité vérifiée',
                              style: TextStyle(
                                  fontSize: 8.5, color: AppColors.success.withValues(alpha: .9))),
                        ],
                      ),
                    ],
                  ),
...
class NetworkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final e in _edges) {
      canvas.drawLine(p[e.$1], p[e.$2], line);
    }
    for (final pt in p) {
      canvas.drawCircle(pt, 2.2, dot);
    }
  }
...
class SolutionsGrid extends StatelessWidget {
  ...
  Widget _mainCard(SolutionMain m, String linkLabel) => Container(
        ...
        child: Column(
          ...
          children: [
            Container(
                ...
                child: Icon(iconOf(m.icon),
                    color: m.featured ? AppColors.gold : AppColors.primaryDark, size: 20)),
            const SizedBox(height: 14),
            Text(m.title,
                style: TextStyle(
                    fontSize: 17,
            ...
            Text(m.subtitle,
                style: TextStyle(
                    fontSize: 11.5, color: m.featured ? AppColors.goldLight : AppColors.textMuted)),
            const SizedBox(height: 10),
            Text(m.text,
                style: TextStyle(
                    fontSize: 11.5,
            ...
            Text('$linkLabel  →',
                style: const TextStyle(
                    color: AppColors.gold, fontSize: 11.5, fontWeight: FontWeight.w700)),
          ],
        ),
      );

  Widget _mini(SolutionMini m) => Container(
        padding: const EdgeInsets.all(18),
        decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(12),
                  ...
                  children: [
                    Text(im.quote,
                        style: const TextStyle(
                            ...
                            height: 1.35)),
                    const SizedBox(height: 14),
                    Container(width: 34, height: 3, color: AppColors.gold),
                  ],
                );
                return c.maxWidth >= 1024
                    ? Row(
...
class SkylinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final e in _b) {
      canvas.drawRect(
          Rect.fromLTWH(e.$1 * size.width, size.height - e.$3 * size.height * .5,
              e.$2 * size.width, e.$3 * size.height * .5),
          paint);
    }
  }
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 24),
                      GoldButton(label: v.cta),
                    ],
                  ),
                ),
              );
              final values = Expanded(
                child: Column(
                  children: [
...
class _FooterState extends State<Footer> {
            ...
            child: Column(
              ...
              children: [
                Wrap(
                  ...
                  children: [_brand(), _nav(), _socials(), SizedBox(width: 300, child: _newsletter())],
                ),
                const SizedBox(height: 40),
                ...
                Wrap(
                  ...
                  children: [
                    Text(widget.f.legal,
                        style: TextStyle(
                            fontSize: 10.5, color: Colors.white.withValues(alpha: .55))),
                    Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Container(width: 26, height: 2, color: AppColors.gold),
                        const SizedBox(width: 10),
                        Text(widget.f.signature,
                            style: TextStyle(
                                fontSize: 10.5, color: Colors.white.withValues(alpha: .75))),
                      ],
                    ),
                  ],
                ),
            ...
            for (final l in widget.nav.links)
              Padding(
                ...
                child: GestureDetector(
                  onTap: () => widget.onNavigate?.call(_sectionOf(l.target)),
                  child: Text(l.label,
                      style: TextStyle(
                          fontSize: 11, color: Colors.white.withValues(alpha: .65))),
                ),
              ),
          ],
        ),
  ...
  Widget _socials() => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(widget.f.socialTitle,
              style: const TextStyle(
                  color: Colors.white, fontSize: 12, fontWeight: FontWeight.w800)),
          const SizedBox(height: 12),
          Row(
            children: [
              for (final s in widget.f.socials)
                Padding(
                  padding: const EdgeInsets.only(right: 10),
                  child: MouseRegion(
                    cursor: SystemMouseCursors.click,
                    child: GestureDetector(
                      onTap: () => SafeLaunch.open(Uri.parse(s.url)),
                      child: Container(
                          width: 34,
                          height: 34,
                          decoration: BoxDecoration(
  ...
  Widget _newsletter() => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(widget.f.nlTitle,
              style: const TextStyle(
                  color: Colors.white, fontSize: 12, fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          Text(widget.f.nlText,
              style: TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: .65))),
          const SizedBox(height: 14),
          Form(
            key: _formKey,
            child: TextFormField(
              ...
              decoration: InputDecoration(
                isDense: true,
                hintText: widget.f.nlPlaceholder,
                hintStyle: TextStyle(
                    color: Colors.white.withValues(alpha: .4), fontSize: 11.5),
          ...
          const SizedBox(height: 8),
          Text(widget.f.privacyNote,
              style: TextStyle(fontSize: 9.5, color: Colors.white.withValues(alpha: .45))),
        ],
      );
}

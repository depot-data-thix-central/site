...
class AdminPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => StreamBuilder<AuthState>(
        ...
        builder: (context, snapshot) {
          ...
          return session == null ? const _Login() : const _Dashboard();
        },
      );
}
...
class _DashboardState extends State<_Dashboard> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            tooltip: 'Importer le contenu initial',
            icon: const Icon(Icons.download),
            onPressed: _ctrl.importSeed,
          ),
          IconButton(
            tooltip: 'Enregistrer le brouillon',
            icon: const Icon(Icons.save),
            onPressed: _ctrl.saving ? null : _ctrl.saveDraft,
          ),
          IconButton(
            tooltip: 'Publier sur le site',
            icon: const Icon(Icons.cloud_upload),
            onPressed: _ctrl.saving ? null : _ctrl.publish,
          ),
          IconButton(
            tooltip: 'Déconnexion',
                  ...
                  children: [
                    if (_ctrl.notice != null)
                      Container(
                        ...
                        child: Text(_ctrl.notice!, style: const TextStyle(fontSize: 12, color: Color(0xFF14532D))),
                      ),
                    _seo(c),
                    _nav(c),
                    _hero(c),
  ...
  Widget _tf(String label, String value, ValueChanged<String> on, {int lines = 1}) => Padding(
        ...
        for (var i = 0; i < c.nav.links.length; i++)
          _itemBox(
            Row(children: [
              ...
              Expanded(
                child: _tf(
                  'Cible (home, about, solutions, impact, vision, actualites, contact)',
                  c.nav.links[i].target,
                  (v) => _edit((x) => x.nav.links[i].target = v.trim()),
                ),
              ),
            ]),
  ...
  Widget _hero(SiteContent c) => _card('Hero', [
        ...
        _tf('Titre (mot doré)', c.hero.highlight, (v) => _edit((x) => x.hero.highlight = v)),
        ...
        for (var i = 0; i < c.impact.stats.length; i++)
          _itemBox(
            Row(children: [
              SizedBox(
                width: 90,
                child: _tf('Valeur', c.impact.stats[i].value, (v) => _edit((x) => x.impact.stats[i].value = v)),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: _tf('Libellé', c.impact.stats[i].label, (v) => _edit((x) => x.impact.stats[i].label = v), lines: 2),
              ),
            ]),
  ...
  Widget _footer(SiteContent c) => _card('Footer', [
        ...
        for (var i = 0; i < c.footer.socials.length; i++)
          _itemBox(
            Column(children: [
              _tf('Icône (linkedin, x, youtube, telegram)', c.footer.socials[i].icon, (v) => _edit((x) => x.footer.socials[i].icon = v.trim())),
              _tf('URL (https uniquement, domaines approuvés)', c.footer.socials[i].url, (v) => _edit((x) => x.footer.socials[i].url = v.trim())),
            ]),
